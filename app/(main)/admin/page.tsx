...
export default function AdminPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      // Nominated books
      const nomBooks = allBooks.filter((b) => nomIds.includes(String(b.id)));
      setNominatedBooks(nomBooks);

      // Display books (non-used)
      const displayBooks = allBooks.filter(
        (b) => !usedIds.includes(String(b.id))
      );
      setAllDisplayBooks(displayBooks);

      // Set default selection
      const finalList = nomBooks.length > 0 ? nomBooks : displayBooks;
      if (finalList.length > 0 && !selectedBookId) {
        setSelectedBookId(String(finalList[0].id));
      }

      // Last event
      if (allEvents.length > 0) {
        setLastEvent(allEvents[0] as EventWithBook);
      }
    } catch {
  ...
  async function handleConfirmEvent(e: React.FormEvent) {
    e.preventDefault();
    if (!selectedBookId || !nextDate) {
      toast.warning("日程と課題本を選択してください");
      return;
    }
  ...
  async function handleContinue(e: React.FormEvent) {
    ...
    try {
      const { error } = await supabase.from("events").insert({
        event_date: contDate,
        book_id: String(lastEvent.book_id),
      });
      if (error) throw error;
  ...
  const finalList =
    nominatedBooks.length > 0 ? nominatedBooks : allDisplayBooks;
  ...
  return (
    <div>
      ...
      <div className="px-4 pt-4 space-y-5">
        ...
        {/* Section 2: Continue previous book */}
            ...
            <form onSubmit={handleContinue} className="space-y-3">
              <div>
                ...
                <input
                  type="date"
                  value={contDate}
                  onChange={(e) => setContDate(e.target.value)}
                  className="w-full px-3 py-2 text-sm border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                  required
                />
              </div>
